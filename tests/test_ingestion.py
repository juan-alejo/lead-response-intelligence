...
def test_mock_places_filters_by_vertical_and_location(tmp_path: Path) -> None:
    f = tmp_path / "places.json"
    f.write_text(json.dumps(_FIXTURE))

    src = GooglePlacesSource(mode="mock", mock_fixture=f)

    manhattan_law = list(src.fetch("law_firm", "manhattan"))
    brooklyn_home = list(src.fetch("home_services", "brooklyn"))
    queens_law = list(src.fetch("law_firm", "queens"))
...
def test_mock_places_respects_limit(tmp_path: Path) -> None:
    ...
    src = GooglePlacesSource(mode="mock", mock_fixture=f)
    result = list(src.fetch("law_firm", "manhattan", limit=3))
...
def test_csv_source_loads_rows(tmp_path: Path) -> None:
    ...
    csv_path.write_text(
        "place_id,business_name,vertical,location,website,phone,email\n"
    ...
    rows = list(CSVSource(csv_path).fetch("law_firm", "manhattan"))
    assert len(rows) == 1
